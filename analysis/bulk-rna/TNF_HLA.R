...
for(cell in cell_types[1]){
  ...
  selectedGenes1  <- colnames(cell_expression_data)[grep("^TNF|^HLA",colnames(cell_expression_data))]
  selectedGenes  <- sample(selectedGenes1,length(baseline))
  ...
  lambda_value <- seq(0.01,0.01,0.01)
...
saveRDS(cellList,file=paste0("figures/TNF_HLA_FromExpRes_andPath_FeatPerm_",zeroThre,".rds"))
saveRDS(resList,file = paste0("Data_Analysis/results/TNF_HLAresultsFromExpRes_andPath_FeatPerm",zeroThre,".rds"))
saveRDS(RandPer,file=paste0("figures/TNF_HLAandFromExpRes_andPath_FeatPerm",zeroThre,".rds"))

ggsave(cellList[[cell]]$plot,file=paste0("figures/TNF_HLAFromExpRes_andPath_FeatPerm_",zeroThre,".pdf"),height = 10,width = 10)
